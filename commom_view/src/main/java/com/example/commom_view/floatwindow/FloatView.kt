...
class FloatView(context: Context) : FrameLayout(context){
    ...
    private fun fixOffset(dx: Int, dy: Int) {
        ...
        val currentOffsetX = (offsetX + dx).coerceAtLeast(0)
        val currentOffsetY = (offsetY + dy).coerceAtLeast(0)
        ...
        offsetX = currentOffsetX.coerceAtMost(maxOffsetX)
        offsetY = currentOffsetY.coerceAtMost(maxOffsetY)
    }

...
public class MediaScanner {
  private static final String[] VIDEO_PROJECTION = new String[]{MediaStore.Video.Media._ID, MediaStore.Video.Media.DATA, MediaStore.Video.Media.DATE_MODIFIED,};
  ...
  private void prescan(String filePath) throws RemoteException {
    ...
    try {
      ...
      c = mProvider.query(MediaStore.Video.Media.CONTENT_URI, VIDEO_PROJECTION, where, selectionArgs, null);
      if (c != null) {
        try {
          while (c.moveToNext()) {
            ...
            if (path.startsWith("/")) {
              ...
              if (!TextUtils.isEmpty(filePath) && !tempFile.exists()) {
                mProvider.delete(MediaStore.Video.Media.CONTENT_URI, where, selectionArgs);
                return;
              }
              path = FileUtils.getCanonical(tempFile);
              String key = mCaseInsensitivePaths ? path.toLowerCase() : path;
              mFileCache.put(key, new FileCacheEntry(MediaStore.Video.Media.CONTENT_URI, rowId, path, lastModified));
            }
          }
        } finally {
          c.close();
  ...
  private class MyMediaScannerClient implements MediaScannerClient {
    ...
    private ContentValues toValues() {
      ...
      if (MediaFile.isVideoFileType(mFileType)) {
        map.put(MediaStore.Video.Media.DURATION, mDuration);
        map.put(MediaStore.Video.Media.LANGUAGE, mLanguage);
        map.put(MediaStore.Video.Media.ALBUM, mAlbum);
        map.put(MediaStore.Video.Media.ARTIST, mArtist);
        map.put(MediaStore.Video.Media.WIDTH, mWidth);
        map.put(MediaStore.Video.Media.HEIGHT, mHeight);
      }
    ...
    private Uri endFile(FileCacheEntry entry) throws RemoteException {
      ...
      if (isVideo) {
        tableUri = MediaStore.Video.Media.CONTENT_URI;
      } else {
        return null;
      }
    ...
    public void addNoMediaFolder(String path) {
      ...
      try {
        mProvider.update(MediaStore.Video.Media.CONTENT_URI, values, MediaStore.MediaColumns.DATA + " LIKE ?", pathSpec);
      } catch (RemoteException e) {

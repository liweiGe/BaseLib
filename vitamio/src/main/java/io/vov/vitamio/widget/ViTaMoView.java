...
/**
 * Displays a video file. The ViTaMoView class can load images from various
 * sources (such as resources or content providers), takes care of computing its
 * measurement from the video so that it can be used in any layout manager, and
 * provides various display options such as scaling and tinting.
 * <p/>
 * ViTaMoView also provide many wrapper methods for
 * {@link }, such as {@link #getVideoWidth()},
 * {@link #setTimedTextShown(boolean)}
 */
public class ViTaMoView extends SurfaceView implements MediaController.MediaPlayerControl {
  ...
  public ViTaMoView(Context context) {
    super(context);
    initVideoView(context);
  }

  public ViTaMoView(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
    initVideoView(context);
  }

  public ViTaMoView(Context context, AttributeSet attrs, int defStyle) {
...
//      return;
  ...
  public SparseArray<MediaFormat> getAudioTrackMap(String encoding) {
    if (mMediaPlayer != null)
      return mMediaPlayer.findTrackFromTrackInfo(MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO, mMediaPlayer.getTrackInfo(encoding));
    return null;
  }
  ...
  public SparseArray<MediaFormat> getSubTrackMap(String encoding) {
    if (mMediaPlayer != null)
      return mMediaPlayer.findTrackFromTrackInfo(MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT, mMediaPlayer.getTrackInfo(encoding));
    return null;
  }

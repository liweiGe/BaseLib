package com.weige.transferee.view.video;
...
/**
 * Created by Vans Z on 2020/5/20.
 */
public class AdaptiveTextureView extends TextureView {
    protected static final String TAG = "ExoVideoView";
    ...
    public int videoDisplayType = VIDEO_IMAGE_DISPLAY_TYPE_ADAPTER;

    public AdaptiveTextureView(Context context) {
        this(context, null);
    }

    public AdaptiveTextureView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AdaptiveTextureView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (videoDisplayType == VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (videoDisplayType == VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                /*原图*/
                height = videoHeight;
                width = videoWidth;
            } else if (videoDisplayType == VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
